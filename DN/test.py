...
class Cnn_test():
    def __init__(self,hyper_params=[]):
        ...
        # comet ml
        self.experiment = Experiment(api_key="jpZwkQl9JhOoOX2WS01GBuYxx",
                                project_name=self.hyper_params.comet_project_name, workspace="udoh-kazuya",disabled=self.hyper_params.comet_disabled)
                ...
                for label_index,set_cur in enumerate(test_sets):
                    ...
                    for im in file_list:
                        if im.endswith(".jpg") or im.endswith(".bmp") or im.endswith(".png"):
                            ...
                            # print('//////////////')
                            # print(psnr_x_)
                            if np.ndim(x_) == 3:  # RGB image
                                img_E_y = utils.rgb2ycbcr(x_, only_y=True)
                                img_H_y = utils.rgb2ycbcr(x_original, only_y=True)
                                psnr_y = utils.calculate_psnr(img_E_y, img_H_y)
                                ssim_y = utils.calculate_ssim(img_E_y, img_H_y)
                                # print(psnr_y)
                            if self.saveImage:
                            ...
                            # print('//////////////')
                            # self.experiment.log_metric("test_psnr_"+im, psnr_x_)
