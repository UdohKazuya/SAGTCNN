...
class Cnn_test():
    def __init__(self,hyper_params=[]):
        ...
        # comet ml
        self.experiment = Experiment(api_key="Id3SlzI5rHOx014GljrulpZc6",
                                project_name=self.hyper_params.comet_project_name, workspace="kaito2ml",disabled=self.hyper_params.comet_disabled)
    ...
    #-----------------------------------experiment.test--------------------------------------
    #----------------------------------------------------------------------------------------------------------------
    def ex_test(self,model_file_name,test_sets,epoch):
        ...
        with self.experiment.test():
            ...
            with torch.no_grad():
                ...
                for label_index,set_cur in enumerate(test_sets):
                    ...
                    self.experiment.log_metric("test_ssim_"+set_cur, ssim_avg, step=epoch)
    ...
    def __call__(self):
        ...
        checpoints = np.arange(200, 401)
        for checpoint in checpoints:
            epoch=checpoint
            # checpoint= 'checkpoint_RetrainGTCNND6_restart_'+str(checpoint)
            checpoint= 'checkpoint_RetrainGTCNND3_restart150_'+str(checpoint)

            self.ex_test(checpoint,self.test_set,epoch)
